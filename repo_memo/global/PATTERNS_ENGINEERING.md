...
所有函数注释使用 doxygen `/** */` 风格，包含以下标签：

| 标签 | 用途 | 适用场景 |
|------|------|---------|
| `@brief` | 一句话功能描述 | 所有函数 |
| `@param` | 参数说明（名称 + 含义 + 约束） | 有参数的函数 |
| `@return` | 返回值语义（参照本文件的返回值约定表） | 有返回值的函数 |
| `@note` | 额外注意事项（副作用、性能特征、安全边界） | 按需 |

新增函数必须包含 doxygen 注释。存量函数在涉及该文件的任务中逐步补充。

示例：
```c
/**
 * @brief  过滤字符串中的控制字符，用于安全日志输出
 * @param  str  输入字符串，可为 NULL
 * @return 过滤后的安全字符串；输入 NULL 时返回 "(null)"
 * @note   内部使用静态缓冲区，调用者不需要释放返回值；不保证线程安全
 */
